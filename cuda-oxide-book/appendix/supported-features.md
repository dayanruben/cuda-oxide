...
| cuda-gdb Local / Argument Inspection | **Partial** | `CUDA_OXIDE_DEBUG=full` emits basic DWARF metadata for whole locals and arguments with scalar or pointer-shaped types. Simple promoted locals use value-based debug records. Rich Rust type trees, projections, destructuring, and exact PHI/block-entry locations are still work in progress. |
